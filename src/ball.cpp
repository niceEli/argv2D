...
void Ball::update(double deltaTime, bool mouseDown, bool mousePressed, std::vector<Tile> tiles, std::vector<Hole> holes, Mix_Chunk *chargeSfx, Mix_Chunk *swingSfx, Mix_Chunk *holeSfx)
{
    if (win)
    {
        ...
        {
            setPos(getPos().x += 0.1f * static_cast<float>(deltaTime), getPos().y);
        }
        ...
        {
            setPos(getPos().x, getPos().y += 0.1f * static_cast<float>(deltaTime));
        }
        else if (getPos().y > target.y)
        {
            setPos(getPos().x, getPos().y -= 0.1f * static_cast<float>(deltaTime));
        }
        setScale(static_cast<float>(getScale().x - 0.001 * deltaTime), static_cast<float>(getScale().y - 0.001 * deltaTime));
        return;
    }
    ...
    if (mousePressed && canMove)
    {
        ...
        setInitialMousePos(static_cast<float>(mouseX), static_cast<float>(mouseY));
    }
    if (mouseDown && canMove)
    {
        ...
        velocity1D = static_cast<float>(SDL_sqrt(SDL_pow(abs(getVelocity().x), 2) + SDL_pow(abs(getVelocity().y), 2)));
        launchedVelocity1D = velocity1D;

        points.at(0).setPos(getPos().x, getPos().y + 8 - 32);
        points.at(0).setAngle(static_cast<float>(SDL_atan2(velocity.y, velocity.x) * (180 / 3.1415) + 90));

        dirX = static_cast<int>(velocity.x / abs(velocity.x));
        dirY = static_cast<int>(velocity.y / abs(velocity.y));
    ...
    else
    {
        ...
        canMove = false;
        setPos(getPos().x + getVelocity().x * static_cast<float>(deltaTime), getPos().y + getVelocity().y * static_cast<float>(deltaTime));
        if (getVelocity().x > 0.0001 || getVelocity().x < -0.0001 || getVelocity().y > 0.0001 || getVelocity().y < -0.0001)
        {
            ...
            if (velocity1D > 0)
            {
                velocity1D -= friction * static_cast<float>(deltaTime);
            }
            else
            {
                velocity1D = 0;
        ...
        for (Tile &t : tiles)
        {
            float newX = getPos().x + getVelocity().x * static_cast<float>(deltaTime);
            float newY = getPos().y;
            ...
            newX = getPos().x;
            newY = getPos().y + getVelocity().y * static_cast<float>(deltaTime);
            if (newX + 16 > t.getPos().x && newX < t.getPos().x + t.getCurrentFrame().w && newY + 16 > t.getPos().y && newY < t.getPos().y + t.getCurrentFrame().h - 3)
            {
